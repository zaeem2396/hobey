...
class Bilship_model extends CI_Model
{
	...
	function getaddressshipp($id)
	{
		$strQuery = "SELECT * from usershipping_address where id=" . $id;
		$result = $this->db->query($strQuery);
		if ($result->num_rows() > 0) {
			$arrRes = $result->row();
			return  $arrRes;
	...
	function rozarpayupdate($content)
	{
		$data['transactionid']  = $content['transactionid'];
		$data['payment_status'] = $content['payment_status'];
		$this->_data = $data;
		$this->db->where('order_id', $content['orderid']);
		if ($this->db->update('ci_orders', $this->_data)) {
			return true;
		} else {
			return false;
