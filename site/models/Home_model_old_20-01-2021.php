...
class Home_Model extends CI_Model
{
	...
	function allproduct($pg_num, $offset,$content)
	{
		...
		$sql = "SELECT p.* FROM product p where p.id <> 0 and is_deleted = 0 ";
			...
			{
				$sql .=" and ( p.name Like '%".$content['subcategory']."%' OR p.product_code Like '%".$content['subcategory']."%') ";
			}*/
			...
			{
					$sql .=  " ORDER BY p.id desc LIMIT ".$offset.",".$pg_num;
			}
		
		
		$query = $this->db->query($sql);
		//echo $this->db->last_query(); die;
		$sql_couint = "SELECT p.* FROM product p where p.id <> 0 and is_deleted = 0 ";
	...
	function relatedproduct_cat($id,$cid)
	{
			...
			if(!empty($cid)){
					...
					$sql .= " AND (";
					for($i = '0'; $i<count( $categorys ); $i++){
					$sql .= "  find_in_set( '".$categorys[$i]."',  p.category_id ) ";
					if($i != count($categorys) -1 ){
					$sql .= " OR ";
					} }
					$sql .= ") ";
...
function allsubcategory($id)
{
	...
	function allbanners()
	{
		$sql = "SELECT * from banner where activepage = '1' order by set_order asc";
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0)
		{
			$result = $query->result();
	...
	public function productminpricesearch($pageurl){
		$sql = "select pro.id as id,pa.* from product pro
				...
				where pro.page_url = '".$pageurl."' and pa.price = (select min(price) from product_attribute where pid = pro.id)";
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0)
		{
			$result = $query->row();
