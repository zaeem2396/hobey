	...
	function tcsdownload()
	{
		...
		if($salesreports!='' && count($salesreports) > 0) {
		 foreach($salesreports as  $prd)
		 {
			 ...
			 if(strtolower($prd->state) != $state){ $igst = round($prductwithoutgst * (0.01)); } else { $igst = '-'; }
			 if(strtolower($prd->state) == $state){ $sgst = round($prductwithoutgst * (0.01)/2); } else { $sgst = '-'; }
			 if(strtolower($prd->state) == $state){ $cgst = round($prductwithoutgst * (0.01)/2); } else { $cgst = '-'; }
	...
	function downloadcommission()
	{
		...
		if($salesreports!='' && count($salesreports) > 0) {
		 foreach($salesreports as  $prd)
		 {
			 ...
			 if($prd->state !=$state){ $igst = number_format($comamt * (18/100),'2','.',''); } else { $igst = '-'; }
			 if($prd->state ==$state){ $sgst = number_format($comamt * (18/100),'2','.','')/2; } else { $sgst = '-'; }
			 if($prd->state ==$state){ $cgst = number_format($comamt * (18/100),'2','.','')/2; } else { $cgst = '-'; }
	...
	function downloadvendorpayment()
	{
		...
		if($salesreports!='' && count($salesreports) > 0) {
		 foreach($salesreports as  $prd)
		 {
			 ...
			 if($get_vendor_name->state !='Goa'){ $igst = number_format($comamt * (18/100),'2','.',''); } else { $igst = '-'; }
	...
	function downloadgstreport(){
		...
		if($salesreports!='' && count($salesreports) > 0) {
		 foreach($salesreports as  $prd)
		 {
			 ...
			 if(strtolower($prd->state) != $state){ $igst = number_format($gstamt,'2','.',''); } else { $igst = '-'; }
			 if(strtolower($prd->state) == $state){ $sgst = number_format($gstamt,'2','.','')/2; } else { $sgst = '-'; }
			 if(strtolower($prd->state) == $state){ $cgst = number_format($gstamt,'2','.','')/2; } else { $cgst = '-'; }
