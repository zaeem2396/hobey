	...
	function edit($id)
	{
		if (is_numeric($id)) {
			...
			$form_field = $data = array(
				...
				'name' => $result->name,
				'sales_office_name' => $result->sales_office_name,
				'sales_office_no' => $result->sales_office_no
			);
			...
			if ($this->input->post('action') == 'edit_category') {
				...
				// }
				$form_field['id'] = $this->input->post("id");
				$form_field['state_id'] = $this->input->post("state_id");
				$form_field['name'] = $this->input->post("name");
				$form_field['sales_office_name'] = $this->input->post("sales_office_name");
				$form_field['sales_office_no'] = $this->input->post("sales_office_no");
				$this->load->library('validation');
				$rules['name'] = "trim|required";
