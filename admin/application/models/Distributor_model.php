<?php
		...
		function addUpload($content)
 		{
			...
			$data['pincode'] = str_replace(' ', '', $content['pincode']);
			//$data['payment_code'] = $content['payment_code'];
			$this->_data = $data;
			if ($this->db->insert('users', $this->_data))
			{
	...
	function editUpload($id, $content)
 		{
			$data['name'] = $content['name'];
			$data['mobile'] = $content['mobile'];
			$data['email'] = $content['email'];
    ...
    function lists($num, $offset, $content)
	{
		...
		$sql = "SELECT * FROM users where id <> 0 and  user_vendor = 2 ";
		...
		$sql_count = "SELECT * FROM users WHERE id <> 0 and  user_vendor = 2 ";
