...
class Pincode_model extends CI_Model {
	...
	function edit($id, $content)
	{
		...
		$this->_data = $data;
		$this->db->where('id', $id);
		if ($this->db->update('pincode', $this->_data))	{
			return true;
		} else {
			return false;
		}
    ...
    function lists($num, $offset, $content)
	{
		...
		$sql = "SELECT * FROM pincode where id <> 0 ";
		...
		$sql_count = "SELECT * FROM  pincode  WHERE id <> 0";
		if($content['categoryname'] !='')
		{
...
function is_already_exist_edit($user,$id)
	...
	function show_pincode_list($cate_id)
	{
	...
	function show_disti_list($cate_id)
	{
		$this->db->where('city_id',$cate_id);
		$this->db->where('user_vendor',2);
