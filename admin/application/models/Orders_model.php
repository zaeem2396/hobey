...
class Orders_model extends CI_Model
{
	...
	public function getspecialOrdersCustomer($order_id = '', $status = '', $startdate = '', $enddate = '', $distributor_id = '', $state_id = '', $city_id = '')
	{
		$this->db->join('users', 'ci_orders.user_id = users.id', 'left');
		$this->db->join('ci_shipping_address', 'ci_orders.order_id = ci_shipping_address.order_id', 'left');
		$this->db->select('users.email as user_email');
		...
		if ($state_id != '') {
			$this->db->where('ci_shipping_address.state', $state_id);
		}
		if ($city_id != '') {
			$this->db->where('ci_shipping_address.city', $city_id);
		}
