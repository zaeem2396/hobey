...
class Material_model extends CI_Model {
	...
	function add($content)
	{
		...
		$this->_data = $data;
		if ($this->db->insert('material', $this->_data))
		{
	...
	function edit($id, $content)
	{
		...
		$this->_data = $data;
		$this->db->where('id', $id);
		if ($this->db->update('material', $this->_data))	{
			return true;
		} else {
			return false;
		}
    ...
    function lists($num, $offset, $content)
	{
		...
		$sql = "SELECT * FROM material where id <> 0 ";
		...
		$sql_count = "SELECT * FROM  material  WHERE id <> 0";
		if($content['categoryname'] !='')
		{
